...
namespace iFrames.BloomKite
{
    public partial class Return_Calc : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ViewState["IsSchemeLoaded"] = "false";

            if (!IsPostBack)
            {
                if (!string.IsNullOrEmpty(Request.QueryString["param"]))
                {
                    hdSchAmfiCode.Value = Request.QueryString["param"];
                    var SchId = AllMethods.getSchemeId(hdSchAmfiCode.Value);
                    var SchDtl = AllMethods.getSchemeDeatils(SchId);

                    LoadNature((int?)SchDtl.Sebi_Nature_ID);
                    LoadSubNature((int)SchDtl.Sebi_Nature_ID, (int)SchDtl.Sebi_Sub_Nature_ID);
                    LoadStructure((int)SchDtl.Structure_ID);
                    LoadAMC((int)SchDtl.MutualFund_ID);
                }
                else
                {
                    //FillNature();
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["param"]))
                {
                    hdSchAmfiCode.Value = Request.QueryString["param"];
                    var SchId = AllMethods.getSchemeId(hdSchAmfiCode.Value);
                    ddlscheme.SelectedIndex = ddlscheme.Items.IndexOf(ddlscheme.Items.FindByValue(SchId.ToString()));
                    ddlscheme.SelectedItem.Value = SchId.ToString();
            ...
            //    if (!string.IsNullOrEmpty(Request.QueryString["param"]))
            //    {
            //        var decryptedString = Decrypt_QueryString(Request.QueryString["param"]);

            //        ddlMode.ClearSelection();
            //        ddlMode.Items.FindByValue(Convert.ToString("Lump Sum")).Selected = true;
            //        ddlMode_SelectedIndexChanged(null, null);

            //        var input = decryptedString.Split('#').ToList();


            //        ddlscheme.ClearSelection();
            //        ddlscheme.Items.FindByValue(Convert.ToString(input[0])).Selected = true;
            //        ddlscheme_SelectedIndexChanged(null, null);

            //        txtLumpfromDate.Text = input[1];
            //        txtLumpToDate.Text = input[2];
            //        txtinstallLs.Text = input[3];

            //        sipbtnshow_Click(null, null);

            //        checkDefaultSelect(Convert.ToInt32(input[0]));
            ...
            //}
            #endregion
            setTextBoxBlank(Convert.ToString(ddlscheme.SelectedValue));
        }
        ...
        protected void checkDefaultSelect(int SchemeId = -1)
        {
            if (SchemeId != -1)
            {
                if (!IsPostBack)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["param"]))
                    {
                        ...
                        var dtResult = new List<SchemeInfo>();

                        if (Cache["dtSchemeInfo"] == null)
                        {
                            dtResult = AllMethods.getAllScheme();
                        }
                        else
                        {
                            dtResult = (List<SchemeInfo>)Cache["dtSchemeInfo"];
                        }

                        if (dtResult != null && dtResult.Count() > 0)
                        {
                            var getMFID = dtResult.Where(x => x.SCHEME_ID == SchemeId).Select(x => x.MutualFund_ID).FirstOrDefault();
                            ddlFundHouse.ClearSelection();
                            ddlFundHouse.Items.FindByValue(getMFID.ToString()).Selected = true;

                            var sebiNature = dtResult.Where(x => x.SCHEME_ID == SchemeId).Select(x => x.Sebi_Nature_ID).FirstOrDefault();
                            if (sebiNature != null)
                            {
                                ddlNature.ClearSelection();
                                ddlNature.Items.FindByValue(Convert.ToString(sebiNature)).Selected = true;
                            ...
                            var sebiSubNature = dtResult.Where(x => x.SCHEME_ID == SchemeId).Select(x => x.Sebi_Sub_Nature_ID).FirstOrDefault();
                            ...
                            var valType = dtResult.Where(x => x.SCHEME_ID == SchemeId).Select(x => x.Options).FirstOrDefault();
                            if (valType != null && valType != -1)
                            {
                                ddlOption.ClearSelection();
                                ddlOption.Items.FindByValue(Convert.ToString(valType)).Selected = true;
                            }
                            else
                            {
                                valType = -1;
        ...
        protected void setTextBoxBlank(string SchemeId = "")
        {
        ...
        #region Load Func From Other
        protected void LoadStructure(int? SelectedId=null)
        {
            DataTable _dt = AllMethods.getOption();
            ...
            {
                //ddlOption.SelectedItem.Value = SelectedId.Value.ToString();
                ddlOption.SelectedIndex = ddlOption.Items.IndexOf(ddlOption.Items.FindByValue(SelectedId.Value.ToString()));
            }
            else
                ddlOption.Items[0].Selected = true;
        }

        protected void LoadNature(int? SelectedId=null)
        {
            DataTable _dt = AllMethods.getSebiNature();
            ddlNature.Items.Clear();
            ddlNature.Items.Add(new ListItem("Select All ", "-1"));
            foreach (DataRow drow in _dt.Rows)
                ddlNature.Items.Add(new ListItem(drow[0].ToString(), drow[1].ToString()));

            if (SelectedId.HasValue)
            {
                //ddlNature.SelectedItem.Value = SelectedId.Value.ToString();
                ddlNature.SelectedIndex = ddlNature.Items.IndexOf(ddlNature.Items.FindByValue(SelectedId.Value.ToString()));
            }
            else
                ddlNature.Items[0].Selected = true;
        }

        protected void LoadSubNature(int natureId = 0, int? SelectedId = null)
        {
            ...
            if (SelectedId.HasValue)
            {
                //ddlSubNature.SelectedItem.Value = SelectedId.Value.ToString();
                ddlSubNature.SelectedIndex = ddlSubNature.Items.IndexOf(ddlSubNature.Items.FindByValue(SelectedId.Value.ToString()));
            }
            else
                ddlSubNature.Items[0].Selected = true;
        }

        protected void LoadAMC(int? SelectedId=null)
        {
            DataTable dtFund;
            dtFund = AllMethods.getFundHouse();
            DataRow drFund = dtFund.NewRow();
            drFund["MutualFund_Name"] = "Select All";
            ...
            if (SelectedId.HasValue)
            {
                //ddlFundHouse.SelectedItem.Value = SelectedId.Value.ToString();
                ddlFundHouse.SelectedIndex = ddlFundHouse.Items.IndexOf(ddlFundHouse.Items.FindByValue(SelectedId.Value.ToString()));
            }
            else
                ddlFundHouse.Items[0].Selected = true;
        }
        ...
        private DataTable SchemeFilteration(bool isAutofill = false)
        {
            List<SchemeInfo> dtSchemeInfo = new List<SchemeInfo>();
            var TempSchInfo = new List<SchemeInfo>();
            if (!IsPostBack && ViewState["IsSchemeLoaded"].ToString() == "false")
            {
                dtSchemeInfo = AllMethods.getAllScheme();

                //var hh = dtSchemeInfo.Where(v => v.SCHEME_ID == 39917).Select(c => c);

                ddlscheme.Items.Clear();
                ddlscheme.Items.Add(new ListItem("-Select Scheme-", "-1"));
                foreach (var drow in dtSchemeInfo)
                {
                    ddlscheme.Items.Add(new ListItem(drow.Sch_Short_Name.ToString(), drow.SCHEME_ID.ToString()));
                }
                ...
                if (Cache["dtSchemeInfo"] == null)
                {
                    Cache.Add("dtSchemeInfo", dtSchemeInfo, null, System.Web.Caching.Cache.NoAbsoluteExpiration, new TimeSpan(24, 0, 0), System.Web.Caching.CacheItemPriority.Default, null);
                }
                else
                {
                    dtSchemeInfo = (List<SchemeInfo>)Cache["dtSchemeInfo"];
                }
                TempSchInfo = new List<SchemeInfo>(dtSchemeInfo);
                ViewState["IsSchemeLoaded"] = "true";
            }
            else
            {
                if (!isAutofill)
                ...
                if (TempSchInfo != null && TempSchInfo.ToList().Count > 0 && Convert.ToInt32(ddlFundHouse.SelectedValue) != -1)
                {
                    TempSchInfo = TempSchInfo.Where(x => x.MutualFund_ID == Convert.ToInt32(ddlFundHouse.SelectedValue)).ToList();
                }

                if (TempSchInfo != null && TempSchInfo.ToList().Count > 0 && Convert.ToInt32(ddlNature.SelectedValue) != -1)
                {
                    TempSchInfo = TempSchInfo.Where(x => x.Sebi_Nature_ID == Convert.ToInt32(ddlNature.SelectedValue)).ToList();
                }

                if (TempSchInfo != null && TempSchInfo.ToList().Count > 0 && Convert.ToInt32(ddlSubNature.SelectedValue) != -1)
                {
                    TempSchInfo = TempSchInfo.Where(x => x.Sebi_Sub_Nature_ID == Convert.ToInt32(ddlSubNature.SelectedValue)).ToList();
                }

                if (TempSchInfo != null && TempSchInfo.ToList().Count > 0 && Convert.ToInt32(ddlOption.SelectedValue) != -1)
                {
                    TempSchInfo = TempSchInfo.Where(x => x.Options == Convert.ToInt32(ddlOption.SelectedValue)).ToList();
                }


                if (TempSchInfo != null && TempSchInfo.ToList().Count > 0)
                {
                    ddlscheme.Items.Clear();
                    ddlscheme.Items.Add(new ListItem("-Select Scheme-", "-1"));

                    //var hh = TempSchInfo.Where(v => v.SCHEME_ID == 4616).Select(c => c);
                    foreach (var drow in TempSchInfo)
                    {
                        ddlscheme.Items.Add(new ListItem(drow.Sch_Short_Name.ToString(), drow.SCHEME_ID.ToString()));
                    }
                ...
                {
                    ddlscheme.Items.Clear();
                    ddlscheme.Items.Add(new ListItem("No Scheme", "-1"));
                }
            }

            if (TempSchInfo != null && TempSchInfo.Count() > 0)
            {
                return TempSchInfo.ToDataTable();
            }
            else
                return new DataTable();
        }

        private DataTable SchemeFilteration(int schWithout)
        {
            List<SchemeInfo> dtSchemeInfo = new List<SchemeInfo>();
            var TempSchInfo = new List<SchemeInfo>();
            if (!IsPostBack)
            ...
            else
            {
                ...
                TempSchInfo = new List<SchemeInfo>(dtSchemeInfo);
                var MFId = TempSchInfo.Where(x => x.SCHEME_ID == schWithout).Select(x => x.MutualFund_ID).FirstOrDefault();


                TempSchInfo = TempSchInfo.Where(x => x.SCHEME_ID != schWithout).ToList();
                //if (TempSchInfo != null && TempSchInfo.ToList().Count > 0 && Convert.ToInt32(ddlFundHouse.SelectedValue) != -1)
                //{
                //    TempSchInfo = TempSchInfo.Where(x => x.MutualFund_ID == Convert.ToInt32(ddlFundHouse.SelectedValue)).ToList();
                //}

                if (TempSchInfo != null && TempSchInfo.ToList().Count > 0 && Convert.ToInt32(MFId) != -1)
                {
                    TempSchInfo = TempSchInfo.Where(x => x.MutualFund_ID == MFId).ToList();
                }
        ...
        {
            MultiView1.ActiveViewIndex = 0;
            lnkTab1.CssClass = "nav-link active";
            lnkTab2.CssClass = "nav-link";
            lnkTab4.CssClass = "nav-link";
            lnkTab3.CssClass = "nav-link";
            lnkTab5.CssClass = "nav-link";
            lnkTab1.Focus();
        }
        ...
        {
            MultiView1.ActiveViewIndex = 1;
            lnkTab1.CssClass = "nav-link";
            lnkTab2.CssClass = "nav-link active";
            lnkTab4.CssClass = "nav-link";
            lnkTab3.CssClass = "nav-link";
            lnkTab5.CssClass = "nav-link";
            lnkTab2.Focus();
        ...
        protected void lnkTab3_Click(object sender, EventArgs e)
        {
            lnkTab1.CssClass = "nav-link";
            lnkTab2.CssClass = "nav-link";
            lnkTab3.CssClass = "nav-link active";
            lnkTab4.CssClass = "nav-link";
            lnkTab5.CssClass = "nav-link";
        ...
        protected void lnkTab4_Click(object sender, EventArgs e)
        {
            ...
            lnkTab1.CssClass = "nav-link";
            lnkTab2.CssClass = "nav-link";
            lnkTab4.CssClass = "nav-link active";
            lnkTab3.CssClass = "nav-link";
            lnkTab5.CssClass = "nav-link";
            lnkTab4.Focus();
            CallChartFromView2();
        ...
        protected void ddlMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ShowRelativeDiv();
            // txtinstall.Text = "";
            if ((!IsPostBack) && (!string.IsNullOrEmpty(Request.QueryString["param"])))
            {
        ...
        protected void ddlscheme_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(ddlscheme.SelectedItem.Value) && Convert.ToInt32(ddlscheme.SelectedItem.Value) != -1 && Convert.ToInt32(ddlscheme.SelectedItem.Value) != 0)
            {
        ...
        {
            LoadSubNature(Convert.ToInt32(ddlNature.SelectedValue));
            FillDropdownScheme();
            //SchemeFilteration();
            resultDiv.Visible = false;
        ...
        public void FillDropdownScheme()
        {
            ...
            //else
            //{
            //    if (string.IsNullOrEmpty(ddlscheme.SelectedValue) && Convert.ToInt32(ddlscheme.SelectedValue) == -1 && Convert.ToInt32(ddlscheme.SelectedValue) == 0)
            //    {
            //        FillDropdown(ddlscheme);
            //    }
            //    else
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        //{
                            ...
                            drpdwn.Items.Add(li);
                        //}
                        SchemeOptions.Add(dr["Scheme_Id"].ToString(), dr["Options"].ToString());
                    }
                    drpdwn.DataBind();
        ...
        public void FillDropdown(Control ddl, string schid)
        {
            try
            {
                ...
                drpdwn.Items.Insert(0, new ListItem("-Select Scheme-", "-1"));
                drpdwn.SelectedIndex = 0;
                //if (dt.Rows.Count > 0)
                //{
                //    DropDownList drpdwn = (DropDownList)ddl;
        ...
        public DataTable FetchSchemeWithout(string schid)
        {
            ...
            try
            {
                dt = SchemeFilteration(Convert.ToInt32(schid));
                #region Commented
        ...
        private void SchemeDataFetch()
        {
            ...
            else
            {
                ...
                var TempSchInfo = new List<SchemeInfo>(dtSchemeInfo);
        ...
        public void CalculateReturn()
        {
            ...
            try
            {
                ...
                //cmdIndex = new SqlCommand("MFIE_SP_INDEX_P2P_ROLLING_RETURN_CLIENT", conn);
        ...
        public DataTable CalculateHistPerf(DateTime EndDate)
        {
            ...
            //cmd = new SqlCommand("MFIE_SP_INDEX_P2P_ROLLING_RETURN_CLIENT", conn);
        ...
        private void PerformanceReturn()
        {
            ...
            try
            {
                ...
                bool IsEquityPlusSchemes = false; // add by syed
                int val = 0;
                #endregion
                ...
                //  cmdIndx = new SqlCommand("MFIE_SP_INDEX_P2P_ROLLING_RETURN_CLIENT", conn);
        ...
        protected void ShowRelativeDiv()
        {
            ...
            if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SIP")
            {
                ...
                GridViewResultLS.Visible = false;
                GridViewLumpSum.Visible = false;
                trSipInvst.Visible = true;
                trLumpInvst.Visible = false;
                trInitialInvst.Visible = false;
                trCategory.Visible = true;
                lblSchemeName.Text = "Scheme Name";
                trTransferTo.Visible = false;
                trBenchmark.Visible = true;
                lblInstallmentAmt.Text = "Installment Amount (₹)";
                lblInstallmentAmt.Visible = true;
                SIP_withdrawl.Visible = true;
                SWP_STP_withdrawl.Visible = false;
                //SWP_STP_withdrawl.Attributes.Add("class", "dspNone");
                //dvWithDrawSP.Attributes.Add("class", "dspNone");
                txtinstall.Visible = true;
                lblDiffDate.Text = "SIP Day";
                ...
                divSTP.Visible = false; txtToDate.Text = ""; txtvalason.Text = "";
                //divSTP.Attributes.Add("class", "dspNone");

                lblDiffDate.Text = "SIP Day";
                ddSIPdate.Visible = true;
                lblDiffDate.Visible = true;
                txtTransferWithdrawal.Visible = false;

                lblTransferWithdrawal2.Visible = false;
                txtTransferWithdrawal2.Visible = false;
                dvWithDrawSPdspn.Attributes.Add("class", "dspNone");
                dvWithDrawSP.Attributes.Remove("class");
                dvWithDrawSP.Attributes.Add("class", "form-group col-xs-12 col-sm-12 col-md-6 col-lg-6");
            }
            else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "LUMP SUM")
            {
                ...
                chkInception4sip.Visible = true;
                tr_SIP_Sinc.Visible = true;
                GridViewResultLS.Visible = true;
                GridViewLumpSum.Visible = true;
                trLumpInvst.Visible = true;
                trCategory.Visible = true;
                lblSchemeName.Text = "Scheme Name";
                trBenchmark.Visible = true;
                lblInstallmentAmt.Text = "Installment Amount (₹)";

                sipGridView.Visible = false;


                lnkTab1.Visible = false; lnkTab2.Visible = false;

                trInitialInvst.Visible = false; trSipInvst.Visible = false;
                trTransferTo.Visible = false;
                
                lblDiffDate.Visible = false;
                // trTransferWithdrawal.Visible = false;
                lblTransferWithdrawal.Visible = false; txtTransferWithdrawal.Visible = false;
                ...
                lblDiffDate.Visible = false;
                ddSIPdate.Visible = false;
                txtTransferWithdrawal.Visible = false;

                lblInitial.Visible = false;
                txtiniAmount.Visible = false;
                lblSIPinit.Visible = false;
                txtIniToDate.Visible = false;
                ...
                dvWithDrawSPdspn.Attributes.Remove("class");
                dvWithDrawSP.Attributes.Remove("class");
                dvWithDrawSP.Attributes.Add("class", "form-group col-xs-12 col-sm-12 col-md-6 col-lg-6");
            ...
            else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SIP WITH INITIAL INVESTMENT")
            {
                ...
                txtiniAmount.Visible = true;
                lblSIPinit.Visible = true;
                txtIniToDate.Visible = true;
                txtTransferWithdrawal.Visible = false;
                ...
                dvWithDrawSPdspn.Attributes.Add("class","dspNone");
                dvWithDrawSP.Attributes.Remove("class");
                dvWithDrawSP.Attributes.Add("class", "form-group col-xs-12 col-sm-12 col-md-12 col-lg-12");
            }
            else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SWP")
            {
                txtinstall.Text = "";
                ...
                GridViewSIPResult.Visible = true;
                lblTransferWithdrawal2.Visible = true; txtTransferWithdrawal2.Visible = true;
                ...
                txtTransferWithdrawal.Visible = true;
            ...
            else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "STP")
            {
                ...
                GridViewResultLS.Visible = false; GridViewLumpSum.Visible = false;
                trSipInvst.Visible = true; trLumpInvst.Visible = false; trInitialInvst.Visible = true;
                trCategory.Visible = true; lblSchemeName.Text = "Transfer From"; trTransferTo.Visible = true;
                ...
                divSTP.Visible = true; txtToDate.Text = ""; txtvalason.Text = "";
                ddlNature.ClearSelection();
        ...
        public void Reset()
        {
            ...
            ////ddlsipbnmark.Items.Clear();
            //txtddlsipbnmark.Text = "";
            //ddlFundHouse.SelectedValue = ddlMode.Items[0].ToString();
            //FillDropdownScheme();

            Response.Redirect("/BloomKite/ReturnCalc.aspx");
        }
        ...
        private void ShowExcel(string FundName)
        {
            ...
            //objFinalstr.Replace(@"class=""grdHead""", "style ='text-align: center;		color: #ffffff;	font-size: 12px;	font-weight:normal;	line-height: 20px;	background:#e46812;	height:25px; width:15px '");
            objFinalstr.Replace(@"class=""grdHead""", "style ='text-align: center;		color: #ffffff;	font-size: 11px;background:#251534;	font-weight:normal;'");
            //objFinalstr.Replace(@"class=""grdRow""", "style=' border: #d0d6db solid 1px;    text-align: center; background-color: #ffffff;        color: #000;    font-size: 11px;    font-weight: normal;'");
            //objFinalstr.Replace(@"class=""grdAltRow""", "style='background-color: #f4f4f4;        color: #000;    font-size: 11px; font-weight: normal;    border: #d0d6db solid 1px;     text-align: center;'");

            var gg = objFinalstr.ToString().Replace("₹", "");
            ...
            try
            {
                ...
                foreach (DataColumn dc in _dt.Columns)
                {
                    ...
                    else if (dc.ColumnName.ToUpper() == "INDEX_VALUE_AMOUNT")
                    {
                        ...
                        chrt.Series[dc.ColumnName].Color = System.Drawing.Color.Green;//#e46812 //System.Drawing.ColorTranslator.FromHtml("#02278b");//
                    ...
                    {
                        CompareToColumn = dc.ColumnName.ToUpper();
                        chrt.Series[dc.ColumnName].Color = System.Drawing.ColorTranslator.FromHtml("#251534");// System.Drawing.Color.Blue;//#e46812
                ...
                string localImgPath = @"BloomKite\Img";
                ...
                var allImageFiles = Directory.GetFiles(HttpContext.Current.Server.MapPath("~") + "BloomKite\\IMG", "BloomKite_SIP_Temp*");
                ...
                imgPath = System.IO.Path.Combine(localImgPath, "BloomKite_SIP_Temp" + Guid.NewGuid().ToString("N") + "_" + tmpChartName);
                ...
                {
                    Session["BLOOM_KITE_GRP_IMG_PATH_STP"] = imgPath;
                    chrtResultSTPTO.SaveImage(imgPath);
                }
                else
                {
                    Session["BLOOM_KITE_GRP_IMG_PATH"] = imgPath;
        ...
        public string GETHtmlFile()
        {
            ...
            switch (RadioButtonListCustomerType.SelectedItem.Text.ToUpper().Trim())
            {
                case "DISTRIBUTOR":
                    if (ddlMode.SelectedItem.Value.ToUpper() == "SIP")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKiteSIPPDFTemplate.html")));// for din regular SundaramPDFtemplate3 normal
                    }
                    else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SWP")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKiteswpPDFTemplate.html")));
                    }
                    else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "STP")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKitestpPDFTemplate.html")));
                    }
                    else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SIP WITH INITIAL INVESTMENT")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKitePDFTemplateInitial.html")));
                    }
                    else
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKiteLsPDFTemplate.html")));
                    }
                    break;
                default:
                    if (ddlMode.SelectedItem.Value.ToUpper() == "SIP")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKiteSIPPDFTemplateWOD.html")));//
                    }
                    else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SWP")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKiteswpPDFTemplateWOD.html")));
                    }
                    else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "STP")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKitestpPDFTemplateWOD.html")));
                    }
                    else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "SIP WITH INITIAL INVESTMENT")
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKitePDFTemplateInitialWOD.html")));
                    }
                    else
                    {
                        strHTML.Append(File.ReadAllText(HttpContext.Current.Server.MapPath("~/BloomKite/Html/BloomKiteLsPDFTemplateWOD.html")));
                    }
                    break;
            }
        ...
        public void CreateHTMLSIP()
        {
            ...
            var allFiles = Directory.GetFiles(HttpContext.Current.Server.MapPath("~") + "BloomKite\\PDF", "BloomKite_*");
            foreach (var f in allFiles)
                if (File.GetCreationTime(f) < DateTime.Now.AddHours(-24))
                    File.Delete(f);
            ...
            try
            {
                ...
                gvFirstTablestr = "<div><table class=" + @"""table table-bordered""" + @" border='0' align='center' width='100%' cellpadding='0' cellspacing='0'>" + gvFirstTablestr.Substring(gvFirstTablestr.IndexOf("<th"));
                ...
                if (ddlMode.SelectedItem.Value.ToUpper().StartsWith("SIP"))
                {
                    gvFirstTablestr = gvFirstTablestr.Replace(@"scope=""col""", "style ='text-align:center;color: #ffffff;font-size: 14px;font-weight: normal;background: #251534;height: 25px;'");
                    gvFirstTablestr = gvFirstTablestr.Replace(@"class=""grdRow""", "style = 'font-size: 14px;;	vertical-align:middle;	text-align:left;height: 25px;'");//font-family:Trebuchet MS;
                }
                else
                {
                    gvFirstTablestr = gvFirstTablestr.Replace(@"scope=""col""", "style ='text-align: center;color: #ffffff;font-size: 14px;font-weight: normal;background: #251534;height: 25px;'");
                    gvFirstTablestr = gvFirstTablestr.Replace(@"class=""grdRow""", "style = 'font-size: 14px;;	vertical-align:middle;	text-align:center;height: 25px;'");//font-family:Trebuchet MS;
                }
                ...
                gvFirstTablestr = gvFirstTablestr.Replace("img/rimg.png", "₹");
                gvFirstTablestr = gvFirstTablestr.Replace(@"border=""1""", "border='1'");
                ...
                string imagePath = Convert.ToString(Session["BLOOM_KITE_GRP_IMG_PATH"]);
                ...
                //added 30 oct 2012
                GridViewSIPResultstr = GridViewSIPResultstr.Replace(@"scope=""col""", "style ='text-align: center;color: #ffffff;font-size: 14px;font-weight: normal;background: #251534;height: 25px;'");

                GridViewSIPResultstr = GridViewSIPResultstr.Replace(@"class=""grdRow""", "style = 'font-size: 14px;;	vertical-align:middle;	text-align:left;	border-bottom:#c6c8ca solid 1px;height: 25px;'");//font-family:Trebuchet MS;
                ...
                if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "LUMP SUM")
                    ...
                    strHTML = strHTML.Replace("<!InstallmentAmount!>", Convert.ToDouble(txtiniAmount.Text).ToString("n0"));
                else if (ddlMode.SelectedItem.Value.Trim().ToUpper() != "STP")
                    strHTML = strHTML.Replace("<!InstallmentAmount!>", Convert.ToDouble(txtinstall.Text).ToString("n0"));
                ...
                if (ddlMode.SelectedItem.Value.Trim().ToUpper().StartsWith("SIP"))
                {
                    ...
                    if (ViewState["gvFirstTableDT"] != null)
                    {
                        ...
                        if (dtTemp.Rows.Count > 0)
                        {
                            ...
                            // var SIP_CAGR = @"<tr>< td class='summery_left'>CAGR Return on your Investment:</td><td class='summery_right'>" + Convert.ToString(Math.Round(sipreturn, 2)) + "%</td></tr>";
                            if (rdbPayout.Checked && IsDividendScheme())
                                strHTML = strHTML.Replace("data-CAGR_RTN", "style='display: none'");
                            else
                            {
                                strHTML = strHTML.Replace("data-CAGR_RTN", "");
                ...
                else if (ddlMode.SelectedItem.Value.Trim().ToUpper() == "LUMP SUM")
                {
                    ...
                    if (lmprdbPayout.Checked && IsDividendScheme())
                        strHTML = strHTML.Replace("data-CAGR_RTN", "style='display: none'");
                    else
                    {
                        strHTML = strHTML.Replace("data-CAGR_RTN", "");
                ...
                Session["BloomKitPdfGUID"] = Guid.NewGuid().ToString();


                path = HttpContext.Current.Server.MapPath("~/BloomKite/PDF/" + "BloomKite" + "_" + Convert.ToString(Session["BloomKitPdfGUID"]) + ".htm");
                ...
                strHTML = null;
                _SimpleConversion("BloomKite");

                string pdfName = string.Empty;
                if (Session["BloomKitPdfGUID"] != null)
                {
                    pdfName = Convert.ToString(Session["BloomKitPdfGUID"]) + ".pdf";
                    Response.ContentType = "Application/pdf";
                    Response.AppendHeader("Content-Disposition", "attachment; filename=BloomKite.pdf");
                    Response.TransmitFile(Server.MapPath("~/BloomKite/PDF/BloomKite_" + pdfName));
                    Response.End();
                }
        ...
        private static void Riskometer(string riskImage, ref string riskImageColor, ref string riskImageColorStmnt)
        {
            ...
            {
                case "LOW RISKOMETER":
                    riskImageColor = "<img src='../IMG/Low.jpg' />";
                    riskImageColorStmnt = riskImageColor + " (Blue)";
                    break;
                case "MODERATELY LOW RISKOMETER":
                    riskImageColor = "<img src='../IMG/Moderately_Low.jpg' />";
                    riskImageColorStmnt = riskImageColor + " (Yellow)";
                    break;
                case "MODERATE RISKOMETER":
                    riskImageColor = "<img src='../IMG/Moderate.jpg' />";
                    riskImageColorStmnt = riskImageColor + " (Brown)";
                    break;
                case "MODERATELY HIGH RISKOMETER":
                    riskImageColor = "<img src='../IMG/Moderately_High.jpg' />";
                    riskImageColorStmnt = riskImageColor + " (Brown)";
                    break;
                case "HIGH RISKOMETER":
                    riskImageColor = "<img src='../IMG/High.jpg' />";
                    riskImageColorStmnt = riskImageColor + " (Brown)";
                    break;
            }
        }
        ...
        public void _SimpleConversion(string CompanyName)
        {
            try
            {
                using (var wk = _GetConverter())
                {
                    ...
                    htmlfile = HttpContext.Current.Server.MapPath("~/" + CompanyName + "/PDF/" + CompanyName + "_" + Convert.ToString(Session["BloomKitPdfGUID"]) + ".htm");
                    ...
                    savepdfpath = HttpContext.Current.Server.MapPath("~") + "\\" + CompanyName + "\\PDF\\" + CompanyName + "_" + Convert.ToString(Session["BloomKitPdfGUID"]) + ".pdf";

                    savehtmlpath = HttpContext.Current.Server.MapPath("~") + "\\" + CompanyName + "\\PDF\\" + CompanyName + "_" + Convert.ToString(Session["BloomKitPdfGUID"]) + ".htm";
        ...
        protected void lnkTab5_Click(object sender, EventArgs e)
        {
            ...
            lnkTab4.CssClass = "nav-link";
            lnkTab3.CssClass = "nav-link";
            lnkTab5.CssClass = "nav-link active";
            lnkTab5.Focus();
        }
