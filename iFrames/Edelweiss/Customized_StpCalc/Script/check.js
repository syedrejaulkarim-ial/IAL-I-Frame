...
function setDate() {
    ...
    if (document.getElementById("ddlMode").value == 'Value STP') {
...
function checkInvestedValue() {
    ...
    else if (mode == 'SWP' || mode == 'Value STP') {
        var text;
        if (mode == 'SWP') text = 'withdrawal';
        else text = 'transfer';

        var intialAmt = document.getElementById("txtiniAmount").value;
        if (isNaN(intialAmt)) {
            alert("Please enter Numeric value.");
...
function checkDateRange() {
    ...
    if (ind > 0) {
        ...
        else if(mode.toUpperCase()!='FLEX STP') {
...
function validateDSP() {
    ...
    else if (document.getElementById("ddlMode").value == 'Value STP') {
    ...
    else if (document.getElementById("ddlMode").value.toUpperCase() == 'FLEX STP') {
        if (!validate_FlexSTP())
            return false;
        else {
            pop();
...
function validate_FlexSTP() {
    ...
    var sipwdamunt = document.getElementById('txtFlexInstallmentAmt').value;

    if (sipwdamunt == "") {
        alert("Value of installment amount cannot be Blank");
        document.getElementById('txtFlexInstallmentAmt').focus();
        return false;
    }

    if (sipwdamunt.indexOf(".") > -1) {
        alert("Please Enter Integral value in installment amount ");
        document.getElementById('txtFlexInstallmentAmt').value = "";
        document.getElementById('txtFlexInstallmentAmt').focus();
        return false;
    }

    if (isNaN(sipwdamunt)) {
        alert("Please enter Numeric value installment amount.");
        document.getElementById('txtFlexInstallmentAmt').value = "";
        document.getElementById('txtFlexInstallmentAmt').focus();
        return false;
    }
        ...
        if (sipwdamunt == 0) {
            alert("Please enter Valid Numeric value installment amount.");
            document.getElementById('txtFlexInstallmentAmt').value = "";
            document.getElementById('txtFlexInstallmentAmt').focus();
            return false;
        }
    ...
    var withamt;
    initamt = document.getElementById("txtiniAmount").value;
    withamt = document.getElementById("txtFlexInstallmentAmt").value;

    if (initamt / withamt < 1) {
        alert('Installment amount cannot be greater than initial amount.');
        document.getElementById("txtFlexInstallmentAmt").focus();
        return false;
    }

    if (initamt / withamt < 1) {
        alert('Enter installment amount for a minimum 1 transaction.');
        document.getElementById("txtFlexInstallmentAmt").focus();
        return false;
    }
    if (document.getElementById("txtfromDate").value == '') {
    ...
    if (ind > 0) {
        var schemeid = document.getElementById('ddlscheme').options[ind].value;
        //commented by syed as it not validating in serevr side
        //if (k <= 1 && dydf < 31 && period == "Monthly") {
        //    alert("STP is allowed for minimum 1 Installment");
        //    document.getElementById('txtfromDate').value = "";
        //    document.getElementById('txtfromDate').focus();
