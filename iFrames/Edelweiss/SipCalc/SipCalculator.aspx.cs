    ...
    public partial class SipCalculator : System.Web.UI.Page
    {
        ...
        protected void FillNature()
        {
            try
            {
                ...
                ddlNature.Items.Add(new ListItem("--", "0"));
                foreach (DataRow drow in _dt.Rows)
                {
                    if (drow["Sebi_Nature"].ToString().ToUpper() != "Solution Oriented".ToUpper() && drow["Sebi_Nature"].ToString().ToUpper() != "Other".ToUpper())
                    {
        ...
        public System.Data.DataTable FetchSchemeNew()
        {
            ...
            List<decimal> ActiveSchemesID = new List<decimal>();

            try
            {
                using (var db = new SIP_ClientDataContext() { CommandTimeout = 6000 })
                {
                    ActiveSchemesID = db.T_SCHEMES_DETAILs.Where(x => x.ACTIVESCHEMEFLAG.Value).Select(x => x.Scheme_ID).ToList();

                }


                using (var scheme = new PrincipalCalcDataContext() { CommandTimeout = 6000 })
                {
                    ...
                    //    && Sch.Nav_Check == 3 && Sch.Launch_Date != null && ActiveSchemesID.Contains(Sch.Scheme_Id)
                    //    select new
                    //    {
                    ...
                    var SqlScheme = (from sch in scheme.T_SCHEMES_MASTERs
                                join t_fund in scheme.T_FUND_MASTERs on sch.Fund_Id equals t_fund.FUND_ID
                                join sebi in scheme.T_SEBI_SCHEMES_NATUREs on t_fund.SEBI_NATURE_ID equals sebi.Sebi_Nature_ID
                                join schop in scheme.T_SCHEMES_OPTIONs on sch.Option_Id equals schop.Option_ID

                                where t_fund.MUTUALFUND_ID == 55 && sch.Nav_Check == 3 && sch.Launch_Date != null //&& ActiveSchemesID.Contains(sch.Scheme_Id)
                    ...
                    if (ddlNature.SelectedIndex != 0)
                    {
                        ...
                        var NatureId = Convert.ToInt32(ddlNature.SelectedValue);
                        if (NatureId == 1 || NatureId == 3 || NatureId == 4)
                        {
                            SqlScheme = SqlScheme.Where(x => x.Sebi_Nature_ID == NatureId);
                        }
                        else
                        {
                            var EddFunds = scheme.T_IFrame_Edelweiss_Natures.Where(c => c.Principal_Nature_Id == NatureId).Select(x => (decimal)x.Fund_Id).ToList();

                            SqlScheme = SqlScheme.Where(p => EddFunds.Contains(p.FUND_ID));
                    ...
                    var scheme_name_1 = SqlScheme.Select(c => new
                    {
                        ...
                        c.Launch_Date,
                        c.Option_ID
                    }).OrderBy(x => x.Sch_Short_Name).ToArray();

                    scheme_name_1 = scheme_name_1.Where(v => ActiveSchemesID.Contains(v.Scheme_Id)).ToArray();
        ...
        public void Reset()
        {
            ...
            txtddlsipbnmark.Text = "";
            FillDropdownScheme();
            chkInception4sip.Checked = false;
        }
