...
{
    public partial class ICRAFundManager : System.Web.UI.Page
    {
        ...
        private void getSchemesList()
        {
            ...
            dtScheme = AllMethods.getScheme(Convert.ToInt32(ddlCategory.SelectedItem.Value),-1,-1,    
   Convert.ToInt32(ddlOption.SelectedValue), Convert.ToInt32(ddlFundHouse.SelectedValue));
            ddlScheme.Items.Clear();
            if (dtScheme != null)
            {
            ...
            {
                ddlScheme.Items.Add(new ListItem("Select", "-1"));
            }
        }
        ...
        public void ExportToExcel(int schemeId, int Year, string Day)
        {
            try
            {
                ...
                string FilePath = Server.MapPath(@"~/Images/favicon-16x16.png");
                ...
                using (var exportData = new MemoryStream())
                {
                    ...
                    Response.AddHeader("Content-Disposition", string.Format("attachment;filename={0}", "ICRAFundManegerExport.xlsx"));
