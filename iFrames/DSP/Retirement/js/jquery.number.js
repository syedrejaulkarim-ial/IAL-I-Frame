    ...
    */
    $.number = function (number, decimals, dec_point, thousands_sep) {
        ...
        if (s[0].length > 3) {
            ...
            //            if (s[0].length > 5)
            //                s[0] = s[0].replace(/\B(?=(?:\d{2})+(?!\d))/g, thousands_sep);

            var x1 = s[0];
            var spl_str = x1.split('');
            var pnt_Incr = 1
            var loopcnt = 1;
            var msg = '';
            var chkSingle = "N"

            for (var cn = 0; cn <= spl_str.length - 1; cn++) {
                if ((spl_str.length - loopcnt) >= 3) {
                    if (pnt_Incr == 2) {
                        msg += spl_str[cn] + ","
                        pnt_Incr = 0
                    }
                ...
                loopcnt += 1
                if (chkSingle == "N" && pnt_Incr == 2) {
                    if ((spl_str.length % 2) == 0) {
                        chkSingle = "Y"; msg += ","; pnt_Incr = 1;
                    }
                }
            }
